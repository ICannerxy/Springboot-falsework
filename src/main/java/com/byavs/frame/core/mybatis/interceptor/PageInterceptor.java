...
/**
 * Created by qibin.long on 2017/8/25.
 */
...
public class PageInterceptor implements Interceptor {
        ...
        try {
            ...
            if (!dialect.skip(ms, parameter, rowBounds)) {
                ...
                if (dialect.beforeCount(ms, parameter, rowBounds)) {
                    ...
                    this.recordStatementCache(ms, countKey);
                ...
                if (dialect.beforePage(ms, parameter, rowBounds)) {
                    ...
                    this.recordStatementCache(ms, pageKey);
                } else {
                    //不执行分页的情况下，也不执行内存分页
                    resultList = executor.query(ms, parameter, RowBounds.DEFAULT, resultHandler, cacheKey, boundSql);
                    this.recordStatementCache(ms, cacheKey);
                }
            } else {
    ...
    private void recordStatementCache(MappedStatement mappedStatement, CacheKey cacheKey) {
        if (!mappedStatement.getConfiguration().isCacheEnabled() || mappedStatement.getCache() == null) {
            return;
        }
        ...
        if (mappedStatement.isUseCache() && cachingManager != null) {
            cachingManager.saveStatementCache(mappedStatement.getId(), mappedStatement.getCache(), cacheKey);
        }
    }
